...
`depth` 只决定**跑哪些阶段**与**默认几位分析师**，不改变辩论轮数——
任何深度下多空辩论都是固定的一轮三人。注意 depth 2 与 3 的默认分析师阵容
是同一批四位，因此这两档之间唯一的差别就是风控阶段，天然是一组单变量对照。

### 所有读取都锚定在分析交易日上

回测 2024-03-01 的决策时，让任何一位成员看到 3 月 5 日的价格、新闻或财报，
得出的结论都准得可疑却毫无意义。这条纪律覆盖全部五类素材，且**由类型系统执行**
而不是靠注释提醒：

| 素材 | 右端锚点 |
|------|----------|
| 行情快照 | `MarketReader.QuoteAsOf(ctx, code, tradeDate)` |
| K 线 | `loadKlines` 的 `DateRange.End` |
| 资讯 / 舆情 | `lookbackRange(tradeDate, days)` |
| 财务 | `Financials(ctx, code, asOf, limit)`，按**披露日**而非报告期过滤 |

`MarketReader` 上**没有** `LatestQuote`——那个方法仍然存在于 `MarketDataRepository`，
只是不对 agent 上下文开放。少一个选项，就少一处需要靠人记住的约定；
要「此刻的价格」的地方（自选股看板、模拟盘估值）各自声明自己的端口。

财报这一条最隐蔽：报告期 2023-12-31 的年报要到次年 4 月底才公布，
按报告期取「最近 N 期」必然会取到尚未存在的数据。判定见
`Financial.DisclosedBy`——有披露日就直接比，缺失时退到法定披露期限
（年报 120 天、其余 62 天）保守估算，**绝不「不知道就放行」**：
存量数据全都没有这个字段，那样写等于这道防护对它们完全不生效。

`engine_asof_test.go` 用一个必须实现完整 `MarketReader` 的 spy 钉住这条纪律。
给端口加一条新数据源时，那个 spy 会编译失败——加的人不得不回来回答
「我这条查询的右端锁在哪一天」。

## 回测与评测：这套系统到底准不准

这是本项目唯一重要的问题，也是最容易自欺的地方。三条命令构成完整链路：

```bash
# ① 积累样本：按「标的 × 交易日」网格补跑历史分析
#    前瞻行情此刻就在库里，样本瓶颈不是时间而是预算——先 dry-run 看清规模
go run . backfill --dry-run --symbols 600519,000001 --from 2026-03-02 --to 2026-06-30 --every 5
go run . backfill --symbols 600519,000001 --from 2026-03-02 --to 2026-06-30 --every 5 --max-total-cost 20

# ② 评分：方向一致率 + 95% 置信区间 + 「无脑全买入」基线
go run . backtest --from 2026-03-02 --to 2026-06-30

# ③ 对照实验：十四人编排 vs 一次直答，配对比较决策方向
#    只花对照组那一半钱——实验组结论直接从已落库的轨迹里读
go run . compare --from 2026-03-02 --to 2026-06-30
```

**一致率永远和样本量、置信区间一起出现，没有例外。** 「58%」单独拿出来说明不了任何事：
n=30 时它的 Wilson 区间约为 [40%, 74%]，与抛硬币无法区分。区间用 Wilson 而非正态近似，
因为后者在 10 战 10 胜时会给出宽度为零的区间——一个看起来无比确凿、实际毫无依据的数字，
而这个项目早期的样本量正好会撞上它。

**基线与系统结论共用同一批样本**，因此两行可以直接比。没有基线的一致率无法解释：
牛市里「无脑全买入」也能有 65%，此时系统的 58% 其实是负贡献。
两条区间大面积重叠时，结论是「当前样本量下分不出高低」，**不是**「打平」。

`compare` 问的是重合率而不是「谁更准」：后者要拿市场当裁判，而在可达的样本量上，
无论多智能体真的强 10pp 还是弱 10pp，你都会得到「不显著」。
重合率是配对设计，市场噪声作为共同项被消掉，n=50 就有可用区间——
它给出的是编排的边际价值**上界**：差异只可能存在于那些不一致的样本里。

对照组（`KindSolo`）刻意不在 `AllKinds` 里、也不进 `NewCrew`：
把它混进编排，这个问题就问不成了。它与实验组走**同一个** `collect`，
这条约束由代码结构保证而非两处逻辑同步——一旦破坏，
测出来的差异里就混进了「谁拿到的数据更好」。

读数也有网页：`GET /agents/evaluations` 与前端的「回测评估」页。
在它之前，全部评测结论只能由作者在自己的终端里敲一条命令看到。

## 成本是一等公民

一次 depth3 分析是几十次模型调用，而回填是无人值守的批量作业。
护栏分三层，缺一层都能让预算烧穿一个数量级：

| 层 | 机制 | 说明 |
|----|------|------|
| 单次分析 | `EngineConfig.MaxCostUSD` → `Orchestrator.WithCostCeiling` | 软护栏：阶段间与每位成员开跑前各检查一次 |
| 单轮工具 | `RuntimeConfig.MaxToolCallsPerRound`（默认 8） | `ToolFanOutLimit` 只是并发信号量，**不控总数** |
| 整批 | `backfill --max-total-cost` | 达到即停，已完成的格子全部保留 |

成本上限是**软**的：检查发生在成员开跑之前，并行阶段可能超出一个批次的量。
要做到分文不差就得在模型调用中途中断，那样付了钱却拿不到任何产出。
它要挡的是数量级失控，不是精确停在 10.00 美元。

超出单轮上限的工具调用**不执行但仍然回一条结果**——
OpenAI 与 Anthropic 都要求每个 `tool_call` 恰好对应一条 `tool` 消息，
少一条整个请求会被判非法，于是「省下几次查询」会变成「这位成员直接失败」。

`backfill` 的 runID 是确定性的（`bf_<symbol>_<date>_d<depth>`），轨迹按 `_id` upsert，
因此断点续跑天然幂等。深度进 ID 是必须的：少了它，对照实验的第二臂会把第一臂
原地覆盖，而两份数据都还在的时候根本发现不了。

## 工具调用有明细，因为工具失败是静默的

`TurnRecord.ToolCalls` 记下每次调用的名字、轮次、成败、耗时与回灌字符数。

这不是可有可无的观测：工具失败在本系统里**不会**让发言失败——
失败被包装成一句「工具执行失败，请不要猜测该数据」回灌给模型，
模型换个角度继续论证，整位分析师照常成功返回。正因为这套降级如此顺滑，
「这份报告是在没拿到财务数据的情况下写出来的」这件事，
除了这份明细，在轨迹、报告与成本统计里全都看不出来。

决策链 API 与前端都透出它，失败的调用标红。
...
报告、回测评估、模拟交易、定时任务、通知、系统配置、用户管理。后两个仅管理员可见——
但那只是前端藏菜单，**权限判定在后端**，别因为前端有门禁就以为服务端可以少判一次。

决策链上每一环都带着模型名、工具轮数、缓存命中与工具调用明细，失败的工具标红。
回测评估页的硬约束是：一致率必须连着样本量与置信区间一起显示，
两条区间重叠时显式标注「无法区分」——那是样本不足的结论，不是打平的结论。
