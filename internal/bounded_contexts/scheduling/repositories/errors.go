// Package repositories 承载定时任务上下文的持久化。
//
...
//
// 每个聚合根一个仓储：ScheduledJobRepository 管 ScheduledJob，
// JobExecutionRepository 管 JobExecution。没有任何一个方法会在同一个事务里同时写这两张表。
...
// subject 是「主语」，用于拼出可读消息，例如 "定时任务(id=xx) 不存在"。
...
// 最后才退回字符串匹配。理由与 analysis 上下文相同：TranslateError 只作用于
